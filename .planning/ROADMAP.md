...
- [ ] **Phase 1: Skeleton + Heuristic Spine + First Comment** - Action posts a real Tier-4 baseline checklist comment on a sandbox issue
...
**Success Criteria** (what must be TRUE):
  ...
  4. Cold-start budget is met: `dist/index.js` is a single ncc-bundled file committed to the repo, `action.yml` declares `using: 'node24'`, and event-to-comment p50 is under 10 seconds on a warm runner.
...
**Goal:** A defensible precision/recall number ships against held-out real OSS issues using the *same* `score()` entrypoint that runs in production. Heuristic weights tune against the 70% training split before the LLM ever lands; the 30% test split is frozen and untouched. Accuracy pillar (30% rubric weight) earned.
**Mode:** mvp
**Depends on:** Phase 2
**Requirements:** BENCH-01, BENCH-02, BENCH-03, BENCH-04, BENCH-05, BENCH-06, BENCH-07
**Success Criteria** (what must be TRUE):
  1. `bench/REPORT.md` is committed to the repo and reports precision, recall, and F1 per issue type and overall, with 95% confidence intervals on N≥100; numbers are produced by replaying scraped fixtures through the *same* `score()` from `src/core/` (not a duplicate scorer).
...
**Success Criteria** (what must be TRUE):
  ...
  3. A fresh-repo install dry-run has been executed *twice* on a sandbox repo before submission (once on a repo with templates, once without); both runs verified end-to-end: comment posted with full hero checklist, label applied, idempotency marker present, no duplicate on a forced re-run.
...
|-------|----------------|--------|-----------|
| 1. Skeleton + Heuristic Spine + First Comment | 0/0 | Not started | - |
...
| **Execution** | 20% | Phase 1 (cold-start budget <10s p50 via JS+ncc, single-file install, zero-config defaults) + Phase 5 (twin fresh-install dry-runs verified) |
